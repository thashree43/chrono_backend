  ...
  try {
    ...
    for (const entry of req.body) {
      ...
      if (!entry.productId || !entry.customerId) {
        return res
          .status(400)
          .json({ message: 'Missing product or customer information' });
      }
      ...
      if (!product) {
        return res
          .status(404)
          .json({ message: `Product not found: ${entry.productId}` });
      }
...
export const emailSalesReport = async (req, res) => {
  try {
    ...
    if (!reportData || !email) {
      return res
        .status(400)
        .json({ message: 'Report data and email are required' });
    }
    ...
    res
      .status(500)
      .json({
        message: 'Failed to send sales report email',
        error: error.message,
      });
  }
};
export const Dahboarddatas = async (req, res) => {
    try {
      const token = req.cookies.jwt;
      if (!token) {
      ...
      // Fetch total sales
      const totalSales = await SalesSchema.aggregate([
      ...
      // Fetch monthly sales
      const monthlySales = await SalesSchema.aggregate([
        { $match: { user: new mongoose.Types.ObjectId(userId) } },
        {
          $group: {
            _id: {
      ...
      res.status(200).json({
        ...
        monthlySales: monthlySales.map(m => ({
          month: new Date(2000, m._id.month - 1).toLocaleString('default', { month: 'short' }),
          total: m.total
        }))
      });
    } catch (error) {
