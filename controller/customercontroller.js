    ...
    const existingCustomer = await CustomerSchema.findOne({ 
      ...
      if (existingCustomer) {
        return res.status(409).json({
          success: false,
          message: existingCustomer.name.toLowerCase() === name.toLowerCase() 
...
export const updateCustomer = async (req,res)=>{
    ...
    try {
        ...
        const updateddatas = await CustomerSchema.findByIdAndUpdate(id,updatedata,{
            new:true,
            runValidators: true,
        })
        if (!updateddatas) {
